...
def get_args_parser():
    ...
    # data
    parser.add_argument("-dn", "--dataset-name", type=str, default="mvtec_ad_2_vial_fruit", help="dataset name")
    parser.add_argument("-ss", "--scales", type=int, nargs="+", help="multiscalfe", default=[4, 8])
    ...
    parser.add_argument("--T", type=int, default=256)  # for image-level inference, DeSTSeg
    # model
    parser.add_argument("-pm", "--pretrained-model", type=str, default='DenseNet', choices=list(MODEL_INFOS.keys()), help="pretrained model")
    parser.add_argument("--category", type=str, default=None, help="train only one category")
    return parser
...
def main(args):
    ...
    if args.category is not None:
        # 只训练 args.category 这一类
        assert args.category in all_categories, f"{args.category} is not a valid category"
        sub_categories = [args.category]
    else:
        # 按原逻辑，可能再从 --sub-categories 里取子集
        sub_categories = all_categories if args.sub_categories is None else args.sub_categories
        for c in sub_categories:
            assert c in all_categories, f"{c} must be in {all_categories}"
...
if __name__ == "__main__":
    ...
    if args.log_path is None:
        args.log_path = f'log/chekpoints/chekpoints_{args.dataset_name}_{args.foreground_dir is not None}'
    main(args)